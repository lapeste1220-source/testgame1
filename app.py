...
#stage{position:absolute;left:50%;top:50%;width:1340px;height:800px;transform-origin:center center;}

/* ===== 시작 타이틀 ===== */
#setup{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding-top:10px;}
.bigtitle{font-size:50px;color:#fff;text-align:center;line-height:1.05;
...
.subt{margin-top:4px;font-size:19px;color:#3a5a8c;background:#ffffffbb;padding:4px 16px;border-radius:14px;}
...
#teamList{max-height:360px;overflow-y:auto;overflow-x:hidden;padding-right:6px;}
#teamList::-webkit-scrollbar{width:10px;}
#teamList::-webkit-scrollbar-thumb{background:#ffcf5f;border-radius:6px;}
#teamList::-webkit-scrollbar-track{background:#fff3d6;border-radius:6px;}
...
#boardArea{position:absolute;left:330px;top:34px;width:720px;height:720px;perspective:1500px;}
...
.cell .ic{font-size:27px;line-height:1;}
.cell .lb{font-size:13px;margin-top:3px;font-weight:bold;}
...
.piece{position:absolute;width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;
...
/* 점수판 */
#score{position:absolute;left:14px;top:46px;width:300px;background:#fffdf7ee;border:4px solid #ffd36b;
...
/* 주사위/조작 */
#ctrl{position:absolute;left:1066px;top:46px;width:260px;display:flex;flex-direction:column;align-items:center;gap:10px;}
...
/* 가운데 승리/파산 조건 패널 */
#centerInfo{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);
  width:320px;background:#fffdf7f2;border:5px solid #ffd36b;border-radius:24px;
  padding:16px 18px;box-shadow:0 9px 0 #e9b94a,0 14px 26px rgba(0,0,0,.18);text-align:center;z-index:18;}
#centerInfo h4{font-size:22px;color:#c0392b;margin-bottom:8px;}
.winrow{display:flex;align-items:center;justify-content:center;gap:10px;margin:8px 0 4px;}
.winrow .v{font-size:30px;font-weight:bold;color:#1f7a3d;min-width:120px;}
.stepbtn{width:40px;height:40px;border:none;border-radius:12px;background:#3a8bff;color:#fff;
  font-size:24px;line-height:1;cursor:pointer;box-shadow:0 3px 0 #2667c9;}
.stepbtn:active{transform:translateY(2px);box-shadow:0 1px 0 #2667c9;}
.centerhint{font-size:13px;color:#9a7b3a;}
.bustrow{font-size:16px;color:#e74c3c;font-weight:bold;margin-top:8px;}

/* 오버레이 퀴즈 (칸 위 덮기) */
#overlay{position:absolute;left:330px;top:34px;width:720px;height:720px;display:none;
  align-items:center;justify-content:center;z-index:60;}
#qcard{width:600px;background:#fffdf7;border:6px solid #ff5fa2;border-radius:26px;padding:22px 26px;
  ...
  <div id="stage">
    ...
    <div id="setup">
      ...
      <div class="panel">
        ...
        <div class="row" style="justify-content:center;margin-top:8px;background:#fff7d6;border-radius:14px;padding:8px;gap:12px;">
          <label style="width:auto;color:#c0392b;font-size:18px;">🏆 승리 목표 점수</label>
          <button class="btn sm b" onclick="setupChangeWin(-50)">－50</button>
          <span id="setupWinV" style="font-size:24px;font-weight:bold;color:#1f7a3d;min-width:90px;display:inline-block;text-align:center;">500점</span>
          <button class="btn sm b" onclick="setupChangeWin(50)">＋50</button>
        </div>
    ...
    <div id="game">
      <div id="score"></div>
      <div id="boardArea"><div id="board"></div><div id="centerInfo"></div></div>
...
/* ---------- 무대 스케일 (스크롤 없이 한 화면) ---------- */
function fit(){
  const s=Math.min(window.innerWidth/1370, window.innerHeight/840);
...
let setupTeams=[];
let setupWin=500;
function setupChangeWin(d){setupWin=Math.max(100,Math.min(2000,setupWin+d));document.getElementById('setupWinV').textContent=setupWin+'점';}
function addTeam(){
...
function goOrder(){
  ...
  G={teams:ts.map(t=>({
     ...
     order, ptr:0, qIdx:0, phase:'roll', curStudent:'', wrongTeams:[], winScore:setupWin};
...
function buildBoard(){
  ...
  board.innerHTML='';
  const size=720, gap=6, cs=(size-gap)/N - gap; // 셀 한 변
...
function cellCenter(idx){
  const size=720,gap=6,w=(size-gap*2)/N;
  const c=LAYOUT[idx];
...
function placePiece(ti,idx){
  ...
  p.style.left=(ctr.x-19+off)+'px';
  p.style.top=(ctr.y-19-(myRank*3))+'px';
}
...
function renderCenter(){
  const el=document.getElementById('centerInfo'); if(!el||!G)return;
  el.innerHTML='<h4>🎯 승리 · 파산 조건</h4>'
   +'<div class="winrow"><button class="stepbtn" onclick="changeWin(-50)">－</button>'
   +'<span class="v">'+G.winScore+'점</span>'
   +'<button class="stepbtn" onclick="changeWin(50)">＋</button></div>'
   +'<div class="centerhint">먼저 도달하면 🏆 승리!</div>'
   +'<div class="bustrow">💥 －100점 이하 → 파산</div>';
}
function changeWin(d){
  if(!G||G.phase==='end')return;
  G.winScore=Math.max(100,Math.min(2000,G.winScore+d));
  renderCenter(); checkEnd();
}
...
function checkEnd(){
  ...
  // 승리
  let winner=G.teams.find(t=>!t.bust && t.score>=G.winScore);
